...
public class StageB implements Stage {
	...
	public StageB(ArrayList<Person> queueList,int totalStaff){
	...
	public StageB(ArrayList<Person> queueList){
		this.queueList = queueList;
		this.totalPerson = queueList.size();
		this.totalStaff = 10;
		this.doneWithList = new ArrayList();
		...
		while(this.queueList.size()>0){
			if(i>0){
				...
				for(int p=this.withStaffList.size()-1;p>=0;p--){
					if(this.withStaffList.get(p).getTimeLeaveB()==time){
	...
	public void randomProcess(){
		...
		while(this.queueList.size()>0){
			...
			for(int k=0;k<this.queueList.size();k++){
				if(this.queueList.get(k).getTimeLeaveA() <=time){
					totalOptions++;
				}
				else{
					break;
			...
			while(this.withStaffList.size()<this.totalStaff  && totalOptions>0){
				...
				if(this.queueList.size()>0  && this.queueList.get(ranIDX).getTimeLeaveA() <= time){
